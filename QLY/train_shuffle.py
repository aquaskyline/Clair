...
def TrainAll(args, m, utils):
    ...
    if args.bin_fn != None:
        ...
        utils.GetTrainingArray(args.tensor_fn,
                               args.var_fn,
                               args.bed_fn)
    tensor_block_index_list = np.arange(int(np.ceil(float(total) / param.bloscBlockSize) ), dtype=int)
    ...
    validation_start_block = int(validationStart / param.bloscBlockSize) - 1
    ...
    XBatch, XNum, XEndFlag = utils.DecompressArray_with_order(XArrayCompressed, datasetPtr, batchSize, total, tensor_block_index_list)
    YBatch, YNum, YEndFlag = utils.DecompressArray_with_order(YArrayCompressed, datasetPtr, batchSize, total, tensor_block_index_list)
    datasetPtr += XNum
    # print(' '.join([str(x) for x in np.append(tensor_block_index_list[:5], tensor_block_index_list[-5:])]))
    while i < param.maxEpoch:
        ...
        XBatch2, XNum2, XEndFlag2 = utils.DecompressArray_with_order(XArrayCompressed, datasetPtr, batchSize, total, tensor_block_index_list)
        YBatch2, YNum2, YEndFlag2 = utils.DecompressArray_with_order(YArrayCompressed, datasetPtr, batchSize, total, tensor_block_index_list)
        ...
        if XEndFlag2 != 0:
            ...
            datasetPtr += XNum
            tensor_block_index_list = shuffle_first_n_items(tensor_block_index_list, validation_start_block)
            logging.info("Shuffled: " + ' '.join([str(x) for x in np.append(tensor_block_index_list[:5], tensor_block_index_list[-5:])]))
    ...
    if args.v2 == True or args.v3 == True:
        datasetPtr = 0
        XBatch, _, _ = utils.DecompressArray_with_order(XArrayCompressed, datasetPtr, predictBatchSize, total)
        bases = []; zs = []; ts = []; ls = []
        ...
        while datasetPtr < total:
            XBatch, _, endFlag = utils.DecompressArray_with_order(XArrayCompressed, datasetPtr, predictBatchSize, total)
            base, z, t, l = m.predict(XBatch)
            bases.append(base); zs.append(z); ts.append(t); ls.append(l)
            datasetPtr += predictBatchSize
            if endFlag != 0:
    ...
    YArray, _, _ = utils.DecompressArray_with_order(YArrayCompressed, 0, total, total)
    if args.v2 == True or args.v3 == True:
