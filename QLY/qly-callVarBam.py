...
def CheckRtCode(signum, frame):
    ...
    if c.CVInstance.returncode != None and c.CVInstance.returncode != 0:
        ...
        sys.exit("call_var_qly.py exited with exceptions. Exiting...")
...
def Run(args):
    ...
    #basedir = "."
    EVCBin = CheckFileExist(basedir + "/../qly-dataPrepScripts/ExtractVariantCandidates.py")
    GTBin = CheckFileExist(basedir + "/../qly-dataPrepScripts/GetTruth.py")
    CTBin = CheckFileExist(basedir + "/../qly-dataPrepScripts/CreateTensor.py")
    CVBin = CheckFileExist(basedir + "/call_var_qly.py")
    ...
    try:
        ...
        if args.activation_only:
            c.CVInstance = subprocess.Popen(\
                shlex.split("%s python %s --chkpnt_fn %s --call_fn %s --sampleName %s --threads %d --activation_only --log_path %s --max_plot %d --parallel_level %d --workers %d %s" %\
                            (taskSet, CVBin, chkpnt_fn, call_fn, sampleName, numCpus, args.log_path, args.max_plot, args.parallel_level, args.workers, "--fast_plotting" if args.fast_plotting else "")),
