    ...
    try:
        ...
        if args.activation_only:
            c.CVInstance = subprocess.Popen(\
                shlex.split("%s python %s --chkpnt_fn %s --call_fn %s --sampleName %s --threads %d --activation_only --log_path %s --max_plot %d --parallel_level %d --workers %d --ref_fn %s %s %s" %\
                            (taskSet, CVBin, chkpnt_fn, call_fn, sampleName, numCpus, args.log_path, args.max_plot, args.parallel_level, args.workers, ref_fn, qual, "--fast_plotting" if args.fast_plotting else "")),
