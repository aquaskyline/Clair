                ...
                if m.group(2) in ("M", "=", "X"):
                    for i in xrange(advance):
                        ...
                        for center in list(activeSet):
                            if availableSlots != 0:
                                availableSlots -= 1
                                centerToAln[center][-1].append( (refPos, 0, refSeq[refPos - (0 if args.refStart == None else (args.refStart - 1))], SEQ[queryPos], STRAND ) )
                    ...
                    for i in xrange(advance):
                        for center in list(activeSet):
                            if availableSlots != 0:
                                availableSlots -= 1
                                centerToAln[center][-1].append( (refPos, 0, refSeq[refPos - (0 if args.refStart == None else (args.refStart - 1))], "-", STRAND ))
                        if refPos in beginToEnd:
                            rEnd, rCenter = beginToEnd[refPos]
                            if rCenter not in activeSet:
                                endToCenter[rEnd] = rCenter
